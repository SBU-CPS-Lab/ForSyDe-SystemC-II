...
struct equalizer : ForSyDe::composite
{
    ...
    equalizer(sc_module_name name) : composite(name)
    {
        auto& audio_filter1 = add(new audio_filter("audio_filter1"));
        ...
        auto& audio_analyzer1 = add(new audio_analyzer("audio_analyzer1"));
        audio_analyzer1.audioIn(filteredInp);
        audio_analyzer1.analyzerOut(distFlag_sig);

        add(new delay("del", abst_ext<AnalyzerMsg>()))(deldistFlag_sig, distFlag_sig);

        add(new mealy("distortion_control1",
                    distortion_control_ns_func,
                    distortion_control_od_func,
                    std::make_tuple(Passed,0)))(override_sig, deldistFlag_sig);

        auto& button_control1 = add(new button_control("button_control1"));
