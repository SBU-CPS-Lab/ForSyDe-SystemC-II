...
UINT32 MPG_Get_Main_Pos ()
{
    ...
    /* Both operands are pointers into g_main_data_vec, so their
     * difference is a small byte offset that fits in a UINT32 -- but the
     * pointers themselves do not, on a 64-bit build. Widen the casts and
     * let the (small) difference narrow on assignment. */
    pos = (UINT32) (((uintptr_t) g_main_data_ptr) - ((uintptr_t) &(g_main_data_vec[0])));
