...
struct ir_channel : ForSyDe::composite
{
            ...
            from_input_vec(no_of_bs)
    {
		...
		for(int i=0;i<(no_of_bs);i++){
			...
			name << "shift" << i;
			shift_vec.push_back(
                &add(new CT::shift(name.str().c_str(), ch_shift[i]))
            );
		}
		...
		for(int i=0;i<(no_of_bs);i++){
			...
			name << "scale" << i;
			scale_vec.push_back(
                &add(new CT::scale(name.str().c_str(), ch_attn[i]))
            );
		}
        ...
        for(int i=0;i<(no_of_bs-1);i++){
			...
			name << "add" << i;
			add_vec.push_back(
                &add(new CT::add(name.str().c_str()))
            );
		}
        
		auto& scale123 = add(new CT::scale("scale123", 1.0));
        //---------------------------------------
