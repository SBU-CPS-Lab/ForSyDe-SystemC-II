...
struct top : ForSyDe::composite
{
	...
	SC_CTOR(top)
	{
		add(new DDE::vsource<bool>("single_event1",
            std::vector<bool>({false,true}),
            std::vector<sc_time>({sc_time(0, SC_SEC),sc_time(15, SC_SEC)})
        ))(on_off);
		
		auto& supervisor1 = add(new supervisor("supervisor1"));
        ...
        auto& discrete_generator1 = add(new discrete_generator(
        ...
        auto& controller1 = add(new controller("controller1"));
        ...
        add(new DDE::comb("expression1",
            [](abst_ext<bool>& fault, const double& voltage) {
                    fault=abst_ext<bool>(voltage >= OVT);
            }
        ))(fault, voltage_expression);
		
		add(new DDE::sink("report1", report_func))(voltage_plot);
	}
